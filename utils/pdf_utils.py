...
def generer_pdf_deces(demande, acte):
    ...
    c.setFont("Helvetica-Bold", 16)
    c.drawCentredString(width / 2, y, "⚰️ ACTE DE DÉCÈS")
    c.setFont("Helvetica", 12)
    y -= 50
    ...
    lignes = [
        f"Demande : {demande.numero_demande}",
        f"Date    : {demande.date_creation.strftime('%d/%m/%Y')}",
        f"Nom     : {acte.prenom_defunt} {acte.nom_defunt}",
        f"Date décès : {acte.date_deces}",
        f"Heure      : {acte.heure_deces or 'N/A'}",
        f"Lieu       : {acte.lieu_deces}",
        f"Cause      : {acte.cause_deces}",
        f"Âge        : {acte.age_deces or 'Non renseigné'}",
        f"Déclarant  : {acte.nom_declarant or 'Non renseigné'}",
        f"Agent      : {acte.agent.get_full_name() if acte.agent else 'N/A'}",
        f"Structure  : {acte.agent.structure.nom if acte.agent and acte.agent.structure else 'N/A'}",
    ]
